<div>
        ...
        <div class="text-gray-900 md:flex md:justify-between md:items-center mt-2">
            ...
            <div class="flex flex-col md:flex-row items-stretch text-center mt-4 md:mt-0">
                @if ( $objeto->user->id != auth()->id() )
                    <button wire:click="$emit('btn_eliminarIntegrante', { id: {{ $objeto->id }}, name: '{{ $objeto->user->name }}' } )" class="bg-red-600 py-2 px-4 rounded-lg text-white text-xs font-bold uppercase">
                        Eliminar
                    </button>
                @endif
            </div>
    ...
    @if ( session()->get('integranteEliminado') )
        <script>
            Swal.fire(
                '¡Listo!',
                'Se eliminó el integrante correctamente.',
                'success',
            )
        </script>
    @endif
